...
class DecisionTree():
    ...
    def calc_entropy(self, dataset):
        ...
        # tính entropy ứng với mỗi nhãn trong dataset đang xét và tổng của chúng
        for enum in range(len(self.labels)):
    ...
    def calc_gain(self, dataset, col_label, lbound):
        # tính entropy trước khi chia nhánh
        info_gain = self.calc_entropy(dataset)
        ...
        # tính entropy của các giá trị có gía trị < ngưỡng đang xét
        info_gain -= (true_count / total_row) * self.calc_entropy(dataset[dataset[:,col_label] <= lbound])
        # tính entropy của các giá trị có gía trị >= ngưỡng đang xét
        info_gain -= (false_count / total_row) * self.calc_entropy(dataset[dataset[:,col_label] > lbound])
        return info_gain
    ...
    def DFS(self, dataset, bound, index, depth):  # sourcery no-metrics
        ...
        calc_entropy = self.calc_entropy(dataset)
        ...
        for col_label in range(dataset.shape[1]):
            ...
            for lbound in np.arange(bound[colid][0], bound[colid][1], 0.05):
                # tính toán information gain, nếu lớn hơn maxinfo_gain hiện tại thì gán maxinfo_gain hiện tại
                # bằng chính nó
                info_gain = self.calc_gain(dataset, col_label, float(lbound))
                # print("col-label: {} - info_gain: {} - lbound: {}".format(col_label, info_gain, lbound))
        ...
        # gọi hàm đệ quy xét nhánh 1 (nhánh có thuộc tính hiện tại < ngưỡng)
        ...
        # gọi hàm đệ quy xét nhánh 2 (nhánh có thuộc tính hiện tại > ngưỡng)
        ...
        return (best_criteria[1], best_criteria[2], child_nodes, maxinfo_gain, calc_entropy)
    ...
    def display_node(self, node, head, depth):
        ...
        for _ in range(depth):
            print(head, end="")
        print("---col: {} <= ".format(node[0]) + "{0:.2f}".format(node[1]) + ", max-split-gain: {0:.1f}".format(node[3]), ", node-gain: {0:.1f}".format(node[4]))
